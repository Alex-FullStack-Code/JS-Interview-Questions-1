...
Answer: 
Object, arguments are array like but not array. 
it has length, can access by index but can't push pop, etc.
...
Answer: 3 
The plus operator between a number and a boolean or 
two boolean will convert boolean to number. 
Hence, true converts to 1 and 
you get result of 2 + 1


Question: What is "2"+true
Answer: 2true 
string concatination happer "2"+"true" -->> 2true


Question: What is the value of -'34'+10
Answer: -24
minus(-) in front of a string is an unary operator 
that will convert the string to a number and 
will make it negative. 
Hence, -'34' becomes, -34 and 
then plus (+) will perform simple addition 
as both the operands are number.

Question: What is the value of +'dude'
Answer: NaN
The plus (+) operator in front of a string is 
an unary operator that will try to convert 
the string to number. 
Here, JavaScript will fail to convert the "dude" 
to a number and will produce NaN.

Question: If you have var y = 1, x = y = typeof x; 
What is the value of x?
Answer: "undefined"


Question: for var a = (2, 3, 5, 6, 1); what is the value of a?
Answer: 1
The comma operator evaluates and
returns the value of the last 1
...
Question: If var a = 2, b = 1 what would be value of a && b
Answer: 1 Takes the last value b = 1
...
//Explaination : 
// Here we are comparing memory location, not an array. 
...
//Explaination : 
// Here we are assigning a to b, where memory location is now same.


let a = [1]; //an array with one element, with value 1
let b = [1];
...
//Explaination : 
// Here we are comparing elements inside an array, 
// not a memory location. We specify the index 
// so that's why element get compared.
...
//Explaination : 
// ...z means destructing happened 
// all the elements are come out from an array/object.
...
//Explaination : 
// We can delete the object property only. 
// We can't able to delete the object(data).


const data = ["piya", "priya", "supriya"];
const [y] = data;
console.log(y); //"priya" means first index, no matter what char i put

//Explaination : 
// We are doing Destructuring here. 
// y is representing the first index only


const data = ["piya", "priya", "supriya"];
const [y, z] = data;
console.log(y, z); //"priya", "priya"
//just get first & second

//Explaination : 
// We are doing destructuring here. 
// y is representing the first index only and 
// z representing the second index.
...
//Explaination : 
// To access any element without taking previous values, 
// we can do just write comma. Propuskam 1, just get second
...
const data = { name:"priya", sex: "female", age: 19 }
const { name, sex } = data; //how to get the name property without . dot operator
console.log(name); //priya
console.log(sex); //female

//Explaination : 
// Here we did object destructuring.
// It's not store in continuos memory location 
// but array store in continuoes memory location so that's why we used comma in an array.
...
//Explaination : 
// using spread operator we use to merge the 2 objects. 
// Three dots will pop out the property from an object 
// and assign inside curly brackets.
...
//Explaination :
// Three dots will pop out the property from an object and 
// assign inside curly brackets. If we didn't do destructing or 
// spread operator then key will be object name(data) and 
// assign the value as whole object into it.
...
//Explaination : 
// If we have same keys while merging then 
// the position of the property will remain same but 
// the value get updated with new value. 
// Because in an objects Keys hould be unique.
...
//Explaination : 
// Function we are calling but it's not present so it will an error.
...
//Explaination : 
// OR operator will find first positive value. 
// Null is a falsy value by default. 
// {} is a positive value. 
// It didn't reach till 20 and null.
...
//Explaination : 
// OR operator will find first positive value. 
// It will print '' because any of the true value 
// didn't found so it will pick last value always.
...
//Explaination : 
// OR operator will find first positive value. 
// It will print [] because empty array/object is a positive.


console.log(Promise.resolve(5)); //Promise {<fulfilled>: 5}

//Explaination : 
// While doing resolve(), itself here a promise. 
// If we pass number/string etc then it will print fulfilled.


console.log("smile" === "smile"); //true

//Explaination : 
// Each emojy contain unicode where 
// we are compairing unicode not the emojy so it's true
...
//Explaination : 
// Hoisting used here. 
// If we use VAR the can use variable before declare. 
// But in LET we can't do that. LET/CONST need declaration first then can use it.
...
// Because of closures we can able to access name which 
// is outside of a function with LET.
...
//Explaination : 
// Template Literal used here. 
// We use here arrow function which is returning a string 
...
//"priya is a good girl" is a string means thruthy value. Nagation of thruthy is falsy value.
...
//Explaination : 
// We are using negation mark(!) which will return 
...
//Explaination : 
// If it's not a number then return True. 
...
//Explaination : 
// Once you use Seal we can't able to add the more keys with values into it.
//But you can modigy the existing key.
...
//Explaination : 
// shift and pop use to remove the elements 
...
//Explaination : 
// Need to divide by 2 and then checking if reminder is zero/one.
...
//Explaination : 
// delete always work with object properties not a object itself.
...
delete data[1];
console.log(data); //["piya", <1 empty item>,"supriya"] //["piya",empty,"supriya"]
console.log(data.length); //3

//Explaination : 
// Whenever element deleted from an object it will create 
// a empty space. Array length will always reain same.
...
//Explaination : 
// using spread operator still we can merge and 
// both values will be present in arrays. 
// But in an object, it will take the lates value and 
// assigned to the 1st position of that key.
...
//Explaination : 
// Here we are using zero time interval. 
// setTimeout is async function so it will execute at last. 
// Firstly all the sync code will get execute. 
// so that's why 100 is assigned to a.
...
//Explaination : a === b gives True. 
// true === -c(number) gives false.
...
//Explaination : 
// We can use VAR variable before its declaration. 
// We didn't assign any value but still by default "undefined" 
// will be store in it.undefined means 
// declared but value didn't initialised.


console.log(a); //not defined

//Explaination : 
// Not defined means variable didn't even declared.Also value is not assogned.


console.log([[[]]]); //[[[]]]
//Explaination : 
// It will print the nested array. 
// We will get 3 nested array and 
// each one have 1 element but at the last array will be empty.
...
//Explaination : 
// Hoisting concept used here. 
// Error because in LET declaration first then only we can use it. 
// but in var we can use brfore its declaration.
...
// Firstly Sync code get executed then 
// async code will start to execute. 
// so i value incremented from 0 to 3 when pointer reaches to 
// setTimeout so it will print 3 thrice time.
...
//Explaination : 
// LET is a block scope. so it will print 0,1,2. Its having own islocated scope.
...
//Explaination : 
// if we write + in front of anything then it will convert into a number.
...
//Explaination : 
// If we wants to access variable with object then 
// use [] notation(. notation will not work).
...
//Explaination : 
// Using assignment operator we are accessing 
...
//Explaination : 
// Can't be declare multiple times with LET.
